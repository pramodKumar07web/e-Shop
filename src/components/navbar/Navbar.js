...
function Navbar() {
  const { userId, userInfo, cartItems } = useContext(UserContext);
  return (
    <>
        ...
        <div className={NavbarStyle.nav_container}>
          ...
          <div className={NavbarStyle.Navbar}>
            ...
            <Link to="/cart-page" className={NavbarStyle.li}>
              <i class="fa-solid fa-cart-shopping"></i>  {cartItems && cartItems.length > 0 && (
              <span className={NavbarStyle.cartNotification}>{cartItems.length}</span>
            )} Cart
